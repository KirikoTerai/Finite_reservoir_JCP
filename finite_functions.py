...
class Site():
    def __init__(self, name: str, energy: float, capacity: int):
        """
        Initialize this site object, with property: name, and site energies
        Arguments:
            name {str} -- Name of the site
            energy {list} -- List of energies of the site
            capacity {int} -- Number of particles the site can occupy up to
        """
        self.name = name
        ...
        self.capacity = capacity    # The number of electrons the site can occupy is equal to the number of reduction potentials
...
class Network():
    ...
    def addSite(self, site: Site):
        ...
        """
        self.num_state *= site.capacity+1        # The total number of possible states is equal to the product of sitecapacities+1 of each site.
    ...
    def getExptvalue(self, pop: np.array, site: Site) -> float:
        ...
        for i in range(self.adj_num_state):
            expt+=self.idx2state(self.allow[i])[site_id]*pop[i]   #sum((number of particle)*(probability))
    ...
    def population(self, pop: np.array, site: Site, occupancy: int) -> float:
        ...
        for i in range(self.adj_num_state):
            #Loop through all the possible states
            if self.idx2state(self.allow[i])[site_id] == occupancy:   #For every state, the number of electrons on each site is known, (ex.)state[0]=[1 2 0 3 2...], state[1]=[0 2 3 1 ...]
